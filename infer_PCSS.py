...
if __name__ == '__main__':
    ...
    parser.add_argument("--load_epo", default=-1, type=int)
    ...
    for iter, pack in enumerate(tqdm(infer_data_loader)):
        model.unpack(pack)
        model.infer_multi(args.load_epo, train_path, dict_path, crf_path, vis=args.vis, dict=args.dict, crf=args.crf)    # original
    ...
    for i in range(20):
        t = i/100.0 + 0.30
        loglist = do_python_eval(dict_path, './data/VOC2012/SegmentationClass', name_list, 21, 'cam', t, printlog=True)
        print('threshold: %.3f\tmIoU: %.3f%%'%(t, loglist['mIoU']))