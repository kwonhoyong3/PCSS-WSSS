...
from evaluation_coco import eval_in_script_coco
...
if __name__ == '__main__':
    ...
    # Dataset
    parser.add_argument('--data-set', default='COCO', type=str, help='dataset')
    parser.add_argument('--data-path', default='./data/COCO2014', type=str, help='dataset')
    
    parser.add_argument("--train_list", default="./coco14/train14.txt", type=str)
    parser.add_argument("--val_list", default="./coco14/train14_small.txt", type=str)
    parser.add_argument("--label_file_path", default="./data/COCO2014/coco_seg_anno2/", type=str)
    ...
    parser.add_argument("--C", default=80, type=int)
    ...
    # Experiments
    parser.add_argument("--model", default='PCSS_eccv24', type=str)
    ...
    parser.add_argument('--scale', default=(0.6,1.2)) #0.3-1.2
    ...
    parser.add_argument("--W", default=[0.4, 0.4], nargs='+', type=float)
    parser.add_argument("--fim_start_epo", default=3, type=int, help='first epoch for FIM process')
    parser.add_argument("--fim_interval_epo", default=3, type=int, help='FIM process interval')
    parser.add_argument("--mpc_cam_start_epo", default=3, type=int, help='first epoch of cam loss for MPC process')
    
    parser.add_argument("--fim_ipc", default=100, type=int, help='number of images per class for FIM process')
    parser.add_argument("--mpc_idx", default=7, type=int, help='merge block index for MPC process')
    parser.add_argument("--mpc_org_ratio", default=0.3, type=float, help='remain ratio of original magnitude in MPC process')

    args = parser.parse_args()

    pyutils.same_seeds(args.seed)
    ...
    freq_dataset,_ = utils.build_dataset_trm(is_train=False, args=args,gen_attn=False)
    ...
    sampler_freq = torch.utils.data.RandomSampler(freq_dataset)
    ...
    for_freq_data_loader = torch.utils.data.DataLoader(
        freq_dataset, sampler=sampler_freq,
        batch_size=64,
        num_workers=args.num_workers,
        pin_memory=args.pin_mem,
        drop_last=False,
    ...
    src_file = "./train_PCSS_coco.py"
    des_file = "./experiments/"+args.name+"/copy_train_coco.py"
    shutil.copy(src_file,des_file)
    ...
    for epo in range(args.epochs):
        ...
        if epo > 9:
            ...
            # Evaluate mIoU
            miou_temp, th_temp = eval_in_script_coco(logger=logger, eval_list=args.val_list, name=args.name, dict_dir='./dict')
        ...
        if args.W[0] > 0:
            if epo >= args.fim_start_epo and epo % args.fim_interval_epo == (args.fim_start_epo % args.fim_interval_epo):
                logger.info('-' * 111)
                logger.info('Epoch ' + epo_str + ' freq_aggregate')
                model.set_phase('freq')
                for iter, pack in enumerate(tqdm(for_freq_data_loader)):
                    model.unpack(pack)
                    is_done_all_class = model.aggregate_freq(epo)
