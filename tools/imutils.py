...
class random_resize():
    ...
    def __call__(self, img, sal=None, get_xy=False, xy=None):
...
def denorm_vgg(img):
    # ImageNet statistics
    # mean_img = [0.485, 0.456, 0.406]
    mean = [122.675, 116.669, 104.008] #RGB
    ...
    proc_img[..., 0] = (imgarr[..., 2] + mean[0])
    proc_img[..., 1] = (imgarr[..., 1] + mean[1])
    proc_img[..., 2] = (imgarr[..., 0] + mean[2])
...
def _crf_with_alpha_rrm(cam_dict, img, alpha=10):
    mean_img = [0.485, 0.456, 0.406]
    std_img = [0.229, 0.224, 0.225]
...
def crf_dl_in_train(img, probs, t=10, scale_factor=1, labels=21):
    import pydensecrf.densecrf as dcrf
    from pydensecrf.utils import unary_from_softmax
...
def crf_dl2(name, probs, t=10, scale_factor=1, labels=21):
    import pydensecrf.densecrf as dcrf
    from pydensecrf.utils import unary_from_softmax



    img_temp = cv2.imread(os.path.join('./data/VOC2012/JPEGImages', name + '.jpg'))
    ...
    d.addPairwiseBilateral(sxy=83, srgb=5, rgbim=np.copy(img), compat=3)  
